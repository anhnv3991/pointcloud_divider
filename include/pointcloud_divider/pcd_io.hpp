...
void CustomPCDReader<PointT>::setInput(const std::string & pcd_path)
{
    ...
    if (!file_.is_open())
    {
        fprintf(stderr, "[%s, %d] %s::Error: Failed to open a file at %s\n", 
                __FILE__, __LINE__, __func__, pcd_path.c_str());
        exit(EXIT_FAILURE);
    }
    ...
    try
    {
        while (input)
        {
            ...
            if (vals[0] == "SIZE")
            {
                ...
                {
                    fprintf(stderr, "[%s, %d] %s::Error: Invalid PCD file format: Number of sizes is different from number of fields. File %s\n",
            ...
            if (vals[0] == "TYPE")
            {
                if (vals.size() - 1 != field_names_.size())
                {
                    fprintf(stderr, "[%s, %d] %s::Error: Invalid PCD file format: Number of types is different from number of fields. File %s\n",
                            __FILE__, __LINE__, __func__, pcd_path_.c_str());
                    exit(EXIT_FAILURE);
                }
            ...
            if (vals[0] == "COUNT")
            {
                if (vals.size() - 1 != field_names_.size())
                {
                    fprintf(stderr, "[%s, %d] %s::Error: Invalid PCD file format: Number of counts is different from number of fields. File %s\n",
                            __FILE__, __LINE__, __func__, pcd_path_.c_str());
                    exit(EXIT_FAILURE);
                }
            ...
            if (vals[0] == "VIEWPOINT")
            {
                if (vals.size() != 8)
                {
                    fprintf(stderr, "[%s, %d] %s::Error: Invalid PCD file format: Viewpoint must contain 7 fields. Number of actual fields: %lu. File %s\n",
...
inline void setFieldReadMetadata(const std::string & field_tag, std::vector<std::string> & field_names, std::vector<size_t> & field_sizes,
                                    std::vector<size_t> & tmp_read_loc, size_t & read_loc, size_t & read_size)
{
...
template<> inline void parsePoint(const char* input, std::vector<size_t> & rsize, std::vector<size_t> & loc, pcl::PointXYZ & output)
{
...
template<> inline void parsePoint(const char* input, std::vector<size_t> & rsize, std::vector<size_t> & loc, pcl::PointXYZI & output)
{
    memcpy(&(output.x), input + loc[0], rsize[0]);
    memcpy(&(output.y), input + loc[1], rsize[1]);
    memcpy(&(output.z), input + loc[2], rsize[2]);
...
template <> inline void parsePoint(const std::string & point_line, pcl::PointXYZ & output)
{
...
template <> inline void parsePoint(const std::string & point_line, pcl::PointXYZI & output)
{
    std::vector<std::string> vals;
